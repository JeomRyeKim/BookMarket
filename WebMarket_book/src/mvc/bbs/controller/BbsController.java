...
public class BbsController extends HttpServlet {
	...
	public void init() throws ServletException {
		...
		System.out.println("BbsconfigFile:"+configFile); //configFile:/WEB-INF/commandHandlerURI.properties
	...
	private void action(HttpServletRequest request, HttpServletResponse response) 
		...
		ActionCommand actionCommand = bbscommandMap.get(command);
		//이동할 view페이지 변수
		String viewPage = null;
		try {
			//command에 해당하는 객체의 action메소드 실행(각 요청의 서비스로 분기 처리) 후 이동페이지 얻기
			viewPage = actionCommand.action(request, response); //다형성을 이용한 메소드 실행
		} catch (Throwable e) {
