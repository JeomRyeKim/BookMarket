...
public class BoardDAO_old {
	...
	private static BoardDAO_old instance;
	...
	public static BoardDAO_old getInstance() {
		if(instance==null) instance = new BoardDAO_old(); //선언만해서 객체가 없기때문에 객체생성
		return instance;
	}
	...
	public void updateBoard(BoardDTO board) {
		...
		if(board.getAttachFile()==null) { //첨부파일이 전송되지 않은 경우
			sql = "update board set id=?,name=?,subject=?,content=?,regist_day=?,ip=? where num=?";
		}else { //첨부파일이 전송된 경우
			sql = "update board set id=?,name=?,subject=?,content=?,regist_day=?,ip=?,attachFile=? where num=?";
		}
